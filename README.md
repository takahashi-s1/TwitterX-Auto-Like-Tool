...
## 頑張った点
- 当初はアカウントのユーザー名とパスワードを直接pythonコード内にに記入する形をとっていたが、セキュリティ面や複数アカウントでの運用を考慮してjsonファイルで管理するようにした（197行目）。
- クラスや関数を定義して処理を抽象化した事で可読性や再利用性を高めた（12行目）。
- try/exceptブロックを頻繁に使用して、エラーが出た際にその箇所や詳細が分かるようにした（46、141、162、179行目）。
- 当初はpyautoGUIのlocateonscreenでいいねのハートマークを認識してクリックする形をとっていたが（160行）、処理の正確性を考慮してキーボードショートカットによる操作（178行目）を追加した。また、前のコードを消さずにmodeで操作方法を変更できるようにした。
- ユーザーのニーズに合わせて設定を柔軟に変更できるようにした点
  - 検索キーワード
  - いいね回数
  - スクロール回数（locateonscreen）
  - アカウント数
  - いいね方法（画像認識、キーボードショートカット）
  - プライバシーモード
...
- 現在Xでは自動化ルールを設けており過度な自動化を規制しています。(https://help.twitter.com/ja/rules-and-policies/x-automation)
  Xサーバーに過負荷を掛けるような過度ないいねの自動化を実施した場合、アカウント凍結やBANされる危険性があります。また、いいねにおけるアクション上限が設けられており、自動化していなくても（手動でも）15分につき50回、1日に1000回以上いいねをすると利用停止されると言われています（自己責任で各自でご判断ください）。
